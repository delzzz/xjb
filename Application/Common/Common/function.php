...
function get_category_id($name)
{
    ...
    $cate_id = M('Category')->where(['name' => $name])->getField('id');
    return $cate_id;
}

/* 根据分类名称获取子分类 */
function get_child_category($name, $field = null)
{
    $pid = get_category_id($name);
    $result = M('Category')->where(['pid' => $pid])->select();
    if (!is_null($field)) {
        $result = M('Category')->field($field)->where(['pid' => $pid])->select();
    }
    return $result;
}

function get_category_byname($name, $fild = null)
{
    $id = get_category_id($name);
    $result = get_category($id);
    return is_null($fild) ? $result : $result[$fild];
}
